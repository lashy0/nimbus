...
static SemaphoreHandle_t s_shared_mutex = NULL;

static bool s_calibration_done = false;
static bool s_ulp_mode = false;

static sensor_ui_runtime_t s_ui_runtime = {
    .charging_screen_shown = false,
    .charging_screen_hide_deadline_us = 0,
};

static sensor_shared_state_t s_shared = {
...
static iaq_phase_t s_iaq_phase = IAQ_PHASE_UNKNOWN;
static uint16_t s_last_logged_iaq = 0xFFFFU;
static uint8_t s_last_logged_accuracy = 0xFFU;
static bool s_last_logged_stabilization_done = false;
static bool s_last_logged_run_in_done = false;
static bool s_last_logged_iaq_valid = false;
static int64_t s_next_iaq_log_time_us = 0;
...
static sensor_sample_result_t sensor_step_read(sensor_worker_state_t* state, bool monitoring)
{
    ...
    if (!bme680_sensor_is_initialized()) {
        return result;
    }
...
static void sensor_step_charging_overlay(bool charging_transition, bool charging_now, int64_t now_us)
{
    if (charging_transition) {
        if (charging_now) {
            ...
            s_ui_runtime.charging_screen_shown = true;
            s_ui_runtime.charging_screen_hide_deadline_us = now_us + ((int64_t)CHARGING_SCREEN_DURATION_MS * 1000LL);
        } else if (s_ui_runtime.charging_screen_shown) {
            if (ui_get_current_screen() == SCREEN_ID_CHARGING) {
                ui_hide_special();
            }
            s_ui_runtime.charging_screen_shown = false;
            s_ui_runtime.charging_screen_hide_deadline_us = 0;
        }
    }

    if (s_ui_runtime.charging_screen_shown && s_ui_runtime.charging_screen_hide_deadline_us > 0 &&
        now_us >= s_ui_runtime.charging_screen_hide_deadline_us) {
        if (ui_get_current_screen() == SCREEN_ID_CHARGING) {
            ui_hide_special();
        }
        s_ui_runtime.charging_screen_shown = false;
...
static void sensor_log_iaq_phase_transition(const bme680_sensor_data_t* data)
{
    ...
    if (phase == s_iaq_phase || phase == IAQ_PHASE_UNKNOWN) {
        return;
    }

    s_iaq_phase = phase;
    ESP_LOGI(TAG, "BME680 IAQ state: %s", sensor_iaq_phase_name(phase));
}
...
static void sensor_log_iaq_snapshot(const bme680_sensor_data_t* data)
{
    ...
    uint16_t delta = 0U;
    if (s_last_logged_iaq != 0xFFFFU) {
        delta = (data->iaq >= s_last_logged_iaq) ? (data->iaq - s_last_logged_iaq)
                                                  : (s_last_logged_iaq - data->iaq);
    }

    bool state_changed = (s_last_logged_iaq == 0xFFFFU) || (data->iaq_accuracy != s_last_logged_accuracy) ||
                         (data->stabilization_done != s_last_logged_stabilization_done) ||
                         (data->run_in_done != s_last_logged_run_in_done) ||
                         (data->iaq_valid != s_last_logged_iaq_valid);
    bool value_changed = (s_last_logged_iaq == 0xFFFFU) || (delta >= IAQ_LOG_DELTA_TRIGGER);
    bool periodic = (s_next_iaq_log_time_us == 0) || (now >= s_next_iaq_log_time_us);
    ...
    s_last_logged_iaq = data->iaq;
    s_last_logged_accuracy = data->iaq_accuracy;
    s_last_logged_stabilization_done = data->stabilization_done;
    s_last_logged_run_in_done = data->run_in_done;
    s_last_logged_iaq_valid = data->iaq_valid;
    s_next_iaq_log_time_us = now + IAQ_LOG_PERIOD_US;
}
...
static void sensor_task(void* arg)
{
    ...
    while (1) {
        ...
        if (s_shared_mutex && xSemaphoreTake(s_shared_mutex, pdMS_TO_TICKS(25)) == pdTRUE) {
            s_shared.latest_sensor_data = latest_sensor_data;
            s_shared.has_sensor_data = has_sensor_data;
            s_shared.battery_info = worker_state.battery_info;
            s_shared.monitoring = monitoring;
            if (charging_transition) {
                s_shared.charging_transition_pending = true;
                s_shared.charging_now = charging_now;
            }
            xSemaphoreGive(s_shared_mutex);
        }
...
static void sensor_ui_timer_cb(lv_timer_t* t)
{
    ...
    if (xSemaphoreTake(s_shared_mutex, pdMS_TO_TICKS(15)) != pdTRUE) {
        ESP_LOGW(TAG, "sensor_ui_timer: shared mutex timeout");
        return;
    }

    snapshot = s_shared;
    if (s_shared.charging_transition_pending) {
        s_shared.charging_transition_pending = false;
    }
    xSemaphoreGive(s_shared_mutex);
...
esp_err_t sensor_runtime_start(void)
{
    ...
    if (!s_shared_mutex) {
        ESP_LOGE(TAG, "Failed to create shared mutex");
        return ESP_FAIL;
    }

    s_ui_timer = lv_timer_create(sensor_ui_timer_cb, SENSOR_UI_TIMER_PERIOD_MS, NULL);
    if (!s_ui_timer) {
        ESP_LOGE(TAG, "Failed to create UI timer");
        return ESP_FAIL;
    }

    BaseType_t task_ret =
        xTaskCreate(sensor_task, "sensor_task", SENSOR_TASK_STACK_SIZE, NULL, SENSOR_TASK_PRIORITY, &s_task_handle);
    if (task_ret != pdPASS) {
