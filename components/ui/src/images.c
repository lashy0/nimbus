...
const img_info_t* get_battery_info(int percent, bool charging)
{
    if (charging) {
        if (percent >= 90)
            return &IMG_INFO_BATT_FULL_CHARGING;
        if (percent >= 60)
            return &IMG_INFO_BATT_3_CHARGING;
        if (percent >= 30)
            return &IMG_INFO_BATT_2_CHARGING;
        return &IMG_INFO_BATT_1_CHARGING;
    } else {
        if (percent >= 90)
            return &IMG_INFO_BATT_FULL_NOT_CHARGING;
        if (percent >= 60)
            return &IMG_INFO_BATT_3_NOT_CHARGING;
        if (percent >= 30)
            return &IMG_INFO_BATT_2_NOT_CHARGING;
...
{
    if (iaq <= 50)
        return &IMG_INFO_ULTRA_HAPPY;
    if (iaq <= 100)
        return &IMG_INFO_HAPPY;
    if (iaq <= 200)
        return &IMG_INFO_ORDINARY;
    if (iaq <= 300)
        return &IMG_INFO_SAD;
    if (iaq <= 400)
        return &IMG_INFO_DIZZY;
...
{
    if (temp < 0)
        return &IMG_INFO_TEMP_MINUS;
    if (temp <= 25)
        return &IMG_INFO_TEMP_NORMAL;
...
{
    if (temp < 0)
        return &IMG_INFO_MINUS;
    if (temp <= 25)
        return &IMG_INFO_NOTHING;
